...
## Phase-specific emphasis: Live verification & DX

- Operates on the integrated root repo, not a clean-room slice. Needs Docker (Postgres + Redis).
- The live `go test -tags=integration ./...` run is mandatory evidence — execute it, capture output. "Documented as unrunnable" is only acceptable if Docker is genuinely absent, with the concrete blocker.
- Fixes here are surgical: infra/wiring/SQL/docs only. Do not change domain logic that phase-1..7 unit tests cover.
- Compose dev credentials are not secrets; still never commit real secrets.
- Migrations must apply cleanly from a fresh database and be idempotent/repeatable.
- Integration tests must fail loudly (not `t.Skip`) when the tag is set and services are down.
- pgx repositories get their first real execution here; treat any SQL/scan/encoding error as a defect to fix, then re-run the suite green.
