...
## Phase-specific emphasis: Docs + integration tests

- Integration tests gated behind `//go:build integration`; document how to run them and what services they need.
- End-to-end test exercises the real workflow: login -> create customer -> create product -> create order -> worker processes -> status PAID, asserting final persisted state.
- Swagger/OpenAPI annotations must match actual request/response/error schemas and the JWT auth header.
- README follows architecture §33 structure and includes the Swagger URL.
- Docs/tests must not silently change API behavior; if a mismatch is found, fix or document it.
