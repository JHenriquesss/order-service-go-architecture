...
func TestE2EWorkflowReachesPaidAndIncrementsMetrics(t *testing.T) {
	...
	beforeProcessed, _ := parseMetricValue(before, "orders_processed_total")
	...
	afterProcessed, ok := parseMetricValue(after, "orders_processed_total")
	if !ok {
		t.Fatal("orders_processed_total missing from /metrics")
	}
	if afterProcessed <= beforeProcessed {
		t.Fatalf("orders_processed_total did not increase: before=%d after=%d", beforeProcessed, afterProcessed)
	}
}
