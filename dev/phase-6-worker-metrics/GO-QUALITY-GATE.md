...
## Phase-specific emphasis: Worker, concurrency, metrics

- Concurrency is the high-risk area. Every goroutine has bounded/documented lifetime and a supervised error path.
- `context.Context` first param for blocking/I/O ops; respect cancellation and shutdown (drain workers cleanly).
- No detached fire-and-forget goroutines. No `time.Sleep` for synchronization. Bounded worker count from config.
- Shared state synchronized; run `go test -race ./...` and record evidence. No copying lock-bearing structs.
- Worker processes only `CREATED` orders (BR-ORD-009); CREATED->PROCESSING->PAID|FAILED. Failure => FAILED + metric + log (BR-ORD-010).
- Status transitions validated centrally; invalid transition => `INVALID_ORDER_STATUS` / 400. Cancel only CREATED, ship only PAID.
- Metrics counters concurrency-safe (atomic or mutex). `/metrics` output matches architecture §22 format.
