...
## Phase-specific emphasis: Order creation

- `OrderStatus` is a defined type with validating parser; created orders start `CREATED` (BR-ORD-005).
- Total = sum of item totals; item total = quantity * unit_price. Money math decimal-safe, not float64.
- Unit price is **copied** from product at creation time (BR-PRD-005), not referenced.
- Transaction boundary: INSERT order + items in one DB tx, COMMIT, **then** publish to Redis. Never publish before commit. Commit ok + publish fail => 500 + logged inconsistency (architecture §20).
- Repository and queue producer are interfaces; unit tests use fakes (no real DB/Redis).
- Reject: inactive customer (`INACTIVE_CUSTOMER`), inactive product (`INACTIVE_PRODUCT`), empty items, quantity < 1.
