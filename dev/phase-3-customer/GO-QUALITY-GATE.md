...
## Phase-specific emphasis: Customer

- `document` uniqueness enforced; duplicate => `DUPLICATE_RESOURCE`, not a 500.
- Deactivate, never physically delete (BR-CUS-004).
- Pagination: validate `page`/`page_size` bounds; return a typed `Page[T]` with total.
- Repository returns a domain not-found signal (sentinel/`(T,bool)`), mapped to `RESOURCE_NOT_FOUND` at handler.
