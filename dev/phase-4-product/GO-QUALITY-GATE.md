...
## Phase-specific emphasis: Product

- Price is `numeric(15,2)`; model as a decimal-safe type (not float64). Reject price <= 0 (BR-PRD-003).
- `sku` uniqueness => `DUPLICATE_RESOURCE`.
- Deactivate, never delete. Pagination as in customer.
- `FindManyByID` returns products for a set of ids (used by orders later) — keep signature clean.
